...
｢業務マニュアルのあの記載、どこにあったっけ｣と探す時間を取られていませんか。

私は 業務マニュアルを 1 ファイルずつ開いて検索する手間を省くために、**PDF・テキスト・Markdown を横断して全文検索し、該当箇所をハイライト表示する** Windows デスクトップアプリ **ManualSearch** を自作しました。

「マニュアル検索」という課題については **生成AI の RAG に投げる** のが最近の主流になっています。私も少し試しましたが、**正確さと回答速度が求められる業務においては、マニュアル原本を直接開いて参照する方が有用** というのが実感です。
...
- Acrobat Reader / Windows 検索 などでは物足りなかった方
...
まずは**手順を一部省略した回答**が返ってくることです。正確さが求められる場面では、**一見正しそうで一部省略されているのが最も危険**です。回答を読む人が省略に気づかず、そのまま作業してしまうリスクがあるためです。
...
もちろん RAG が完全に不要だと言いたいわけではありません。**探索的な問いやざっくりした要約には RAG が、正確さと原文確認・即応性が要る業務には全文検索＋ハイライトが向く** という使い分けが大事だと考えています。
...
```bash
git clone https://github.com/yokamoto5742/ManualSearch
cd ManualSearch
python -m venv venv
...
RAG が主流になった今だからこそ、 **「決められた文章を、決められたまま、すぐ確認したい」** という地味だが根強いニーズに、クラシックな全文検索＋ハイライトはまだまだ十分応えられると思います。トレンドに物足りなさを感じている方の、実装と道具選びの手がかりになれば幸いです。
